...
router.get('/documents/status', async (req, res) => {
  try {
    const checks = await Promise.all([
      execAsync('python3 -c "import PyPDF2"').then(() => true).catch(() => false),
      execAsync('python3 -c "from docx import Document"').then(() => true).catch(() => false),
      execAsync('python3 -c "import openpyxl"').then(() => true).catch(() => false),
      execAsync('python3 -c "from pptx import Presentation"').then(() => true).catch(() => false),
      execAsync('which pdftotext').then(() => true).catch(() => false),
      execAsync('which tesseract').then(() => true).catch(() => false),
    ]);
