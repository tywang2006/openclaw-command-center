...
// Fast lookup map for O(1) getCatalogEntry (type → entry)
...
let dynamicCatalogMap: Map<string, CatalogEntryWithCategory> | null = null
...
export function buildDynamicCatalog(assets: LoadedAssetData): boolean {
  ...
  dynamicCatalogMap = new Map(visibleEntries.map((e) => [e.type, e]))
...
export function getCatalogEntry(type: string): CatalogEntryWithCategory | undefined {
  // Check internal catalog map first (O(1) lookup, includes all variants)
  if (internalCatalogMap) {
    return internalCatalogMap.get(type)
  }
  // Fallback: use dynamic catalog map or linear scan on static catalog
  if (dynamicCatalogMap) {
    return dynamicCatalogMap.get(type)
  }
