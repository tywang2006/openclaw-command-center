...
export class OfficeState {
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      const notWalkable = !outOfBounds && !isWalkable(ch.tileCol, ch.tileRow, this.tileMap, this.blockedTiles)
      if (outOfBounds || notWalkable) {
  ...
  reassignSeat(agentId: number, seatId: string): void {
    ...
    seat.assigned = true
    ch.seatId = seatId
    // Check if already at destination (manhattan distance <= 1)
    const dist = Math.abs(ch.tileCol - seat.seatCol) + Math.abs(ch.tileRow - seat.seatRow)
    if (dist <= 1) {
      // Already at or adjacent to seat — sit down immediately
    ...
    if (path.length > 0) {
      ...
      // No path found — sit down anyway (prevents stuck state)
      ch.state = CharacterState.TYPE
      ch.dir = seat.facingDir
      ch.frame = 0
      ch.frameTimer = 0
